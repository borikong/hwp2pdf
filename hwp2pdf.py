...
def get_save_root(dl):
    root = Tk().withdraw()
    save_folder = tkinter.filedialog.askdirectory(title=dl);
    print(save_folder)
    return save_folder

def make_doc(hwp_root,save_root):

    root_dir=get_save_root("hwp 목록 선택")
    save_dir=get_save_root("저장 폴더 선택")
    # root_dir=hwp_root
    # save_dir=save_root

    os.chdir(root_dir)  # hwp 파일이 있는 폴더로 이동
    ...
    ##저장 폴더
    BASE_DIR = save_dir  # 한/글은 파일 열거나 저장할 때 전체경로를 입력해야 하므로, os.path.join(BASE_DIR, i) 식으로 사용할 것
